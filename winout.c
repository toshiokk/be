...
{
	win_select_cur_sub_win(WIN_IDX_WHOLE);	// set whole of central win to 'cur_sub_win'
	central_win = cur_sub_win;		// &sub_win_rects[WIN_IDX_WHOLE];
	for (int sub_win_idx = 0; sub_win_idx < SUB_WINS; sub_win_idx++) {
		win_select_cur_sub_win(sub_win_idx);
		switch (sub_win_idx) {
		default:
		case WIN_IDX_WHOLE:
			cur_sub_win->top = 0 + win_depth * 1;
			cur_sub_win->left = 0 + win_depth * 2;
			cur_sub_win->lines = LIM_MIN(0, tio_get_lines() - (win_depth * 1 * 2));
			cur_sub_win->columns = LIM_MIN(0, tio_get_columns() - (win_depth * 2 * 2));
			break;
		case WIN_IDX_SUB_WHOLE:
			cur_sub_win->top = central_win->top + central_win_get_top_win_lines();
			cur_sub_win->left = central_win->left + 0;
