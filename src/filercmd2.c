...
void dof_view_file()
{
	...
	if (S_ISREG(get_cfv_file_info(-1)->st.st_mode)) {
		const char *file_name = get_cfv_file_name(-1);
...
PRIVATE int dof_open_file_(int flags)
{
	...
	for (int file_idx = get_first_file_idx_selected(); file_idx >= 0;
	 file_idx = get_next_file_idx_selected(file_idx)) {
		if (S_ISREG(get_cfv_file_info(file_idx)->st.st_mode)) {
			if (load_file_name_upp_low_(get_cfv_file_name(file_idx),
...
{
	char file_path[MAX_PATH_LEN+1];
	if (chk_inp_str_ret_val_filer(input_string_pos(get_fv_from_another_pane()->cur_dir,
	 file_path, MAX_PATH_LEN, HISTORY_TYPE_IDX_DIR,
	 (update1 == 0) ? _("Copy to (update):") : _("Copy to (force):")))) {
		return 0;				// cancelled
	}
	int use_target_dir = 0;		// "-t target-dir"
	if (get_cfv_files_selected() >= 2) {
		use_target_dir = 1;
	}
	int exit_status = 0;
...
void dof_rename_file()
{
	...
	strlcpy__(file_name, get_cfv_file_name(-1), MAX_PATH_LEN);
...
void dof_trash_file()
{
	...
	int files_selected;
	if ((files_selected = get_cfv_files_selected()) == 0)
		ret = ask_yes_no(ASK_YES_NO_QUIT, _("Trash file %s ?"), get_cfv_file_name(-1));
	else
		ret = ask_yes_no(ASK_YES_NO_QUIT, _("Trash %d files ?"), files_selected);
	if (ret <= 0) {
		return;
	}
	int exit_status = 0;
...
void dof_delete_file()
{
	...
	int files_selected;
	if ((files_selected = get_cfv_files_selected()) == 0)
		ret = ask_yes_no(ASK_YES_NO_QUIT, _("Delete file %s ?"), get_cfv_file_name(-1));
	else
		ret = ask_yes_no(ASK_YES_NO_QUIT, _("Delete %d files ?"), files_selected);
	if (ret <= 0) {
		return;
	}
	int exit_status = 0;
...
void dof_mark_to_delete_file()
{
	...
	int files_selected;
	if ((files_selected = get_cfv_files_selected()) == 0)
		ret = ask_yes_no(ASK_YES_NO_QUIT, _("Mark file %s to be Deleted later ?"),
		 get_cfv_file_name(-1));
	else
		ret = ask_yes_no(ASK_YES_NO_QUIT, _("Mark %d files to be Deleted later ?"),
		 files_selected);
	if (ret <= 0) {
		return;
	}
...
void dof_size_zero_file()
{
	...
	if ((files_selected = get_cfv_files_selected()) == 0) {
		ret = ask_yes_no(ASK_YES_NO_QUIT,
		 _("Make size of file %s 0 ?"), get_cfv_file_name(-1));
	} else {
		ret = ask_yes_no(ASK_YES_NO_QUIT,
		 _("Make size of %d files 0 ?"), files_selected);
	}
	if (ret <= 0) {
		return;
	}
...
void dof_clip_file_name()
{
	dof_clip_file_path_(0);		// "file-or-dir"
}
void dof_clip_file_path()
{
...
flf_dprintf("cur_dir: [%s], next_file\n",
 get_fv_from_cur_pane()->cur_dir, get_fv_from_cur_pane()->next_file);
	...
	separate_last_dir_or_file_from_path(
...
void dof_real_path()
{
	...
	file_info_t *f_info = get_cfv_file_info(-1);
	if (S_ISLNK(f_info->lst.st_mode)) {
		...
		if (S_ISREG(f_info->st.st_mode)) {
			// if the path is file, get directory of the file
			separate_path_to_dir_and_file(dir, dir, NULL);
		}
		// change to the directory
	} else {
		// get real path of the current directory
