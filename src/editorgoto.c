...
//------------------------------------------------------------------------------
void doe_switch_to_prev_buffer()
{
...
void doe_switch_to_next_buffer()
{
	if (switch_epc_buf_to_next_buf(1, 0) == 0) {
...
PRIVATE int switch_to_prev_buffers();
PRIVATE int switch_to_next_buffers();

void doe_switch_to_top_buffer()
{
	if (IS_NODE_TOP_MOST(get_epc_buf())) {
		// already top of buffers
		return;
	}
	if (switch_epc_buf_to_top_buf() == 0) {
		return;
	}
	post_cmd_processing(NULL, CURS_MOVE_HORIZ, LOCATE_CURS_NONE, UPDATE_SCRN_ALL_SOON);
	disp_status_bar_done(_("Switched to the first buffer"));
}
void doe_switch_to_bot_buffer()
{
...
void doe_switch_to_prev_bufs_boundary()
{
	if (IS_NODE_TOP_MOST(get_epc_buf())) {
		// already top of buffers
		// ==> switch to the bottom-buffer of the previous buffers
		if (switch_to_prev_buffers()) {
			switch_epc_buf_to_bot_node();
		}
		return;
	}
	switch_epc_buf_to_top_node();
	post_cmd_processing(NULL, CURS_MOVE_HORIZ, LOCATE_CURS_NONE, UPDATE_SCRN_ALL_SOON);
	disp_status_bar_done(_("Switched to the first buffer"));
}
void doe_switch_to_next_bufs_boundary()
{
	if (IS_NODE_BOT_MOST(get_epc_buf())) {
		// already bottom of buffers
		// ==> switch to the top-buffer of the next buffers
		if (switch_to_next_buffers()) {
			switch_epc_buf_to_top_node();
		}
		return;
	}
...
//------------------------------------------------------------------------------
PRIVATE char file_pos_before_jump[MAX_PATH_LEN+1] = "";
PRIVATE char prev_file_pos_before_jump[MAX_PATH_LEN+1] = "";
void memorize_cur_file_pos_before_jump()
{
	memorize_cur_file_pos_into_str(file_pos_before_jump);
}
int memorize_prev_file_pos_if_changed()
{
	char cur_file_pos[MAX_PATH_LEN+1] = "";
	memorize_cur_file_pos_into_str(cur_file_pos);
	if (strcmp(cur_file_pos, file_pos_before_jump) != 0) {
		strlcpy__(prev_file_pos_before_jump, file_pos_before_jump, MAX_PATH_LEN);
		return 1;
	}
	return 0;
}
void doe_return_to_prev_file_pos()
{
	...
	recall_file_pos_from_str(prev_file_pos_before_jump);
	if (memorize_prev_file_pos_if_changed()) {
		doe_refresh_editor();
...
#define MIN_FREE_MEM_KB					(100 * 1000)	// 100 MB
	...
	recall_file_pos_from_str(file_pos_str1);
	return files;	// 0--
}
		...
		if (get_file_line_col_from_str(history, file_path, NULL, NULL)) {
			if (compare_file_path_from_tail(file_path, file_name) == 0) {
				return load_file_in_string_(history,
				 TUL0 | OOE0 | MOE0 | RDOL0 | FOLF0 | LFH0 | RECURS0 | MFPL0);
			}
		}
	}
	return -1;		// nothing loaded nor selected
...
{
	file_path = quote_file_path_s(file_path);
	if (col_num <= 0) {
		if (line_num <= 0) {
			// /path/to/file.ext
...
int switch_epc_buf_to_top_node()
{
	be_bufs_t *bufs = get_bufs_contains_buf(get_epc_buf());
	if (IS_NODES_EMPTY(bufs)) {
		set_epc_buf(NODES_TOP_ANCH(bufs));
	} else {
		set_epc_buf(NODES_TOP_NODE(bufs));
	}
	return 1;
}
int switch_epc_buf_to_bot_node()
{
	be_bufs_t *bufs = get_bufs_contains_buf(get_epc_buf());
	if (IS_NODES_EMPTY(bufs)) {
		set_epc_buf(NODES_BOT_ANCH(bufs));
	} else {
		set_epc_buf(NODES_BOT_NODE(bufs));
	}
	return 1;
}
...
int switch_epc_buf_to_another_buf()
{
	if (switch_epc_buf_to_next_buf(0, 0) == 0) {
		if (switch_epc_buf_to_prev_buf(0, 0) == 0) {
