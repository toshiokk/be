...
void locate_curs_y_in_edit_win(locate_cursor_to_t locate_curs)
{
	...
	case LOCATE_CURS_JUMP_FORWARD:	// locate cursor keeping screen if possible.
		// In search string,
		// Case-A: When the next cursor position is in the current view,
		//         it does not move contents and locate cursor in it.
		// Case-B: When the next cursor position is out of the current view,
		//         it does move contents and locate cursor at the center of screen.
		if ((disp_y_preferred = get_disp_y_after_cursor_move()) >= 0) {
			// Case-A: current line is in the previous screen
		} else {
			// Case-B: current line is out of the previous screen
			// LOCATE_CURS_CENTER
			switch (locate_curs) {
			default:
	...
	set_EPCBVC_CURS_Y(disp_y_preferred);
...
void update_text_x_to_keep_after_cursor_move(cursor_horiz_vert_move_t cursor_move)
{
	if ((cursor_move == CURS_MOVE_HORIZ) || (cursor_move == CURS_MOVE_JUMP)) {
		...
		set_EPCBVC_CURS_X_TO_KEEP(start_col_idx_of_wrap_line(
		...
		set_EPCBVC_CLBI(end_byte_idx_of_wrap_line_le(EPCBVC_CL->data, wl_idx,
		 EPCBVC_CURS_X_TO_KEEP, -1));
		int cursor_x_in_text = start_col_idx_of_wrap_line(
		 EPCBVC_CL->data, EPCBVC_CLBI, -1);
...
void update_min_text_x_to_keep(int text_x)
{
	...
	if (min_text_x_to_keep != EPCBVC_MIN_TEXT_X_TO_KEEP) {
		set_EPCBVC_MIN_TEXT_X_TO_KEEP(min_text_x_to_keep);
