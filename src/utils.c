...
#define YYMMDD_HHMMSSPMMM_LEN		(6+1+6+1+3)				// "991231-235959.999"
#define YYYYMMDD_HHMMSS_LEN			(8+1+6)					// "20241009-235959"
#define YYMMDD_HHMMSS_LEN			(6+1+6)					// "241009-235959"
#define HHMMSS_LEN					6						// "235959"
PRIVATE const char *get_yyyysmmsdd_hhcmmcss(time_t abs_time, char *buf);
PRIVATE const char *get_yyyymmdd_hhmmss(time_t abs_time, char *buf);
...
const char *cur_cdate()
{
	...
	strlcpy__(buf_date, &(get_yyyysmmsdd_hhcmmcss(time(NULL), NULL)[2]), YYSMMSDD_LEN);
	return buf_date;
}
const char *cur_ctime()
{
	static char buf_time[HHCMMCSS_LEN+1];
	strlcpy__(buf_time, &(get_yyyysmmsdd_hhcmmcss(time(NULL), NULL)[11]), HHCMMCSS_LEN);
	return buf_time;
}
const char *cur_hhmmss()
{
	static char buf_time[HHMMSS_LEN+1];
	strlcpy__(buf_time, &(get_yyyymmdd_hhmmss(time(NULL), buf_time)[8+1]), HHMMSS_LEN);
	return buf_time;
}
const char *cur_yymmdd_hhmmss()
{
	static char buf_time[YYMMDD_HHMMSS_LEN+1];
	strlcpy__(buf_time, &(get_yyyymmdd_hhmmss(time(NULL), buf_time)[2]), YYMMDD_HHMMSS_LEN);
	return buf_time;
}
const char *cur_yymmdd_hhmmsspmmm()
{
	static char buf_time[YYMMDD_HHMMSSPMMM_LEN+1];
	struct timeval tv;
	struct timezone tz;
	gettimeofday(&tv, &tz);
	snprintf_(buf_time, YYMMDD_HHMMSSPMMM_LEN+1, "%s.%03d",
	 &(get_yyyymmdd_hhmmss(tv.tv_sec, buf_time)[2]), (int)(tv.tv_usec / 1000 % 1000));
	return buf_time;
}
//------------------------------------------------------------------------------
...
{
	static char hhcmmcss[HHCMMCSS_LEN+1];
	strlcpy__(hhcmmcss, &(get_yyyysmmsdd_hhcmmcss(time(NULL), NULL)[11]), HHCMMCSS_LEN);
	return hhcmmcss;
}
const char *cur_hhcmmcsspmmm()
{
#define HHCMMCSS_MMM_LEN		(8+1+3)
	...
	snprintf_(buf_time, HHCMMCSS_MMM_LEN+1, "%s.%03d",
	 &(get_yyyysmmsdd_hhcmmcss(tv.tv_sec, NULL)[11]), (int)(tv.tv_usec / 1000 % 1000));
	return buf_time;
}
const char *cur_hhcmmcsspuuuuuu()
{
#define HHCMMCSS_UUUUUU_LEN		(8+1+6)
	...
	struct timezone tz;
	gettimeofday(&tv, &tz);
	snprintf_(buf_time, HHCMMCSS_UUUUUU_LEN+1, "%s.%06d",
	 &(get_yyyysmmsdd_hhcmmcss(tv.tv_sec, NULL)[11]), (int)tv.tv_usec);
	return buf_time;
}

PRIVATE const char *get_yyyysmmsdd_hhcmmcss(time_t abs_time, char *buf)
{
	static char yyyysmmsdd_hhcmmcss[YYYYSMMSDD_HHCMMCSS_LEN+1];
	struct tm tm_;
	struct tm *tm;

	if (buf == NULL) {
		buf = yyyysmmsdd_hhcmmcss;
	}
	if (abs_time == 0) {
		//             0123456789012345678
		strcpy__(buf, "----/--/-- --:--:--");
...
PRIVATE const char *get_yyyymmdd_hhmmss(time_t abs_time, char *buf)
{
	static char yyyymmdd_hhmmss[YYYYMMDD_HHMMSS_LEN+1];
	struct tm tm_;
	struct tm *tm;
