 ...
 const char *func, const char *label, const char *format, va_list list)
{
	...
	switch (time) {
	case DTSM:
		snprintf_(buf_time, MAX_PATH_LEN, "SEC %s", get_sssssspmmm(buffer));
		break;
	case DTSU:
		snprintf_(buf_time, MAX_PATH_LEN, "SEC %s", get_ssspuuuuuu(buffer));
		break;
	case DTHM:
		strlcpy__(buf_time, cur_hhcmmcsspmmm(), MAX_PATH_LEN);
		break;
	case DTHMT:
		snprintf_(buf_time, MAX_PATH_LEN, "%s:%s", cur_hhcmmcsspmmm(), get_tty_name_file_part());
		break;
	case DTHU:
		strlcpy__(buf_time, cur_hhcmmcsspuuuuuu(), MAX_PATH_LEN);
		break;
	case DTYH:
		strlcpy__(buf_time, cur_yymmdd_hhmmss(), MAX_PATH_LEN);
		break;
	case DTYHM:
		strlcpy__(buf_time, cur_yymmdd_hhmmsspmmm(), MAX_PATH_LEN);
		break;
	default:
		break;
	}
